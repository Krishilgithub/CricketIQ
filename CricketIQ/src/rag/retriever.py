...
def get_con() -> Optional[duckdb.DuckDBPyConnection]:
    """Return a read-only DuckDB connection, or None if DB doesn't exist."""
    try:
        cfg = get_config()
    ...
    except:  # bare except catches duckdb C-extension IOException
        return None


@traceable(run_type="retriever", name="Entity & Schema Retrieval")
def extract_entities(query: str, con: Optional[duckdb.DuckDBPyConnection]) -> dict:
    """Extract teams, players, and venues mentioned in the query."""
    if con is None:
...
def execute_sql(query: str) -> str:
    ...
    con = get_con()
    if con is None:
        return "⚠️ Database not available — the analytics database is not deployed in this environment."
    try:
